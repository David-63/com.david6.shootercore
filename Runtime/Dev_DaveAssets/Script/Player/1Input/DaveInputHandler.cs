...
namespace Dave6.ShooterFramework.Input
{
    public class DaveInputHandler : MonoBehaviour
    {
        [Header("인풋 액션 에셋")]
        [SerializeField] private InputActionAsset DaveInputs;
        ...
        private void Awake()
        {
            _basicMap = DaveInputs.FindActionMap("Basic", throwIfNotFound: true);
            _UIMap = DaveInputs.FindActionMap("UI", throwIfNotFound: true);
        ...
        private void SubscribeBasicActions()
        {
            ...
            _sprintAction.performed += ctx => OnStartSprint();
